...
const signInCustomerSchema = z.object({
...
function CustomerEmailAuthForm({
  ...
  const schema = isSignUp ? signUpCustomerSchema : signInCustomerSchema;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-sm mx-auto">
      ...
      <div className="space-y-4">
        ...
        />
        {isSignUp && (
          <p className="text-xs text-gray-600 leading-relaxed -mt-1">{PASSWORD_REQUIREMENT_HINT}</p>
        )}
      ...
      <p className="text-center text-sm text-gray-600">
        ...
        <button type="button" onClick={onToggle} className="text-[#c29112] hover:text-[#c29112] font-medium">
  ...
  return (
    <div className="w-full">
      ...
      <CustomerEmailAuthForm
        key={isSignUp ? "customer-signup" : "customer-signin"}
        isSignUp={isSignUp}
        onToggle={() => setIsSignUp((v) => !v)}
      />
    </div>
  );
}
